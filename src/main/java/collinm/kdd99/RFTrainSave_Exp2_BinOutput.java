...
public class RFTrainSave_Exp2_BinOutput {
	
	/**
	 * Meant to be trained with binary data and tested with non-binarized data.
	 * 
	 * @param args
	...
	public static void main(String[] args) {
		...
		List<Pair<String, String>> perfTups = new ArrayList<>();
		for (Row r : predxns.select("attack_type", "predicted_attack_type_label").collect())
			perfTups.add(Pair.with(r.getString(0), r.getString(1)));
		test.unpersist();

		// Write metrics
		Kdd99Util.writePairs(perfTups, outputDir);
